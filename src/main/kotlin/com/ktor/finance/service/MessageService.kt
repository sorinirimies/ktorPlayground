...
class MessageService : MessageApi {
    ...
    override suspend fun getAllMessages(): List<Message> = dbQuery {
      com.ktor.finance.model.Messages.selectAll().map { toMessage(it) }
    }
    ...
    override suspend fun getMessage(id: Int): Message? = dbQuery {
      com.ktor.finance.model.Messages.select { Messages.id.eq(id) }.mapNotNull {
        toMessage(it)
      }.singleOrNull()
    }
    ...
    override suspend fun deleteMessage(id: Int): Boolean = dbQuery {
      com.ktor.finance.model.Messages.deleteWhere { Messages.id eq id } > 0
    }
}